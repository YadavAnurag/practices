import { Component, OnInit } from '@angular/core';
import { IBook } from '../book-service/book';
import { BookService } from '../book-service/book.service';

@Component({
  selector: 'app-book-list',
  templateUrl: './book-list.component.html',
  styleUrls: ['./book-list.component.css']
})
export class BookListComponent implements OnInit {

  localBooks: IBook[] = [
    {
      "bookID": 1,
      "title": "Harry Potter and the Half-Blood Prince (Harry Potter  #6)",
      "authors": "J.K. Rowling-Mary GrandPré",
      "average_rating": 4.56,
      "isbn": 439785960,
      "language_code": "eng",
      "ratings_count": 1944099,
      "price": 230
    },
    {
      "bookID": 2,
      "title": "Harry Potter and the Order of the Phoenix (Harry Potter  #5)",
      "authors": "J.K. Rowling-Mary GrandPré",
      "average_rating": 4.49,
      "isbn": 439358078,
      "language_code": "eng",
      "ratings_count": 1996446,
      "price": 231
    },
    {
      "bookID": 3,
      "title": "Harry Potter and the Sorcerer's Stone (Harry Potter  #1)",
      "authors": "J.K. Rowling-Mary GrandPré",
      "average_rating": 4.47,
      "isbn": 439554934,
      "language_code": "eng",
      "ratings_count": 5629932,
      "price": 232
    },
    {
      "bookID": 4,
      "title": "Harry Potter and the Chamber of Secrets (Harry Potter  #2)",
      "authors": "J.K. Rowling",
      "average_rating": 4.41,
      "isbn": 439554896,
      "language_code": "eng",
      "ratings_count": 6267,
      "price": 233
    },
    {
      "bookID": 5,
      "title": "Harry Potter and the Prisoner of Azkaban (Harry Potter  #3)",
      "authors": "J.K. Rowling-Mary GrandPré",
      "average_rating": 4.55,
      "isbn": 5765,
      "language_code": "eng",
      "ratings_count": 2149872,
      "price": 234
    },
    {
      "bookID": 8,
      "title": "Harry Potter Boxed Set  Books 1-5 (Harry Potter  #1-5)",
      "authors": "J.K. Rowling-Mary GrandPré",
      "average_rating": 4.78,
      "isbn": 439682584,
      "language_code": "eng",
      "ratings_count": 38872,
      "price": 235
    },
    {
      "bookID": 9,
      "title": "Unauthorized Harry Potter Book Seven News: \"Half-Blood Prince\" Analysis and Speculation",
      "authors": "W. Frederick Zimmerman",
      "average_rating": 3.69,
      "isbn": 976540606,
      "language_code": "en-US",
      "ratings_count": 18,
      "price": 236
    },
    {
      "bookID": 10,
      "title": "Harry Potter Collection (Harry Potter  #1-6)",
      "authors": "J.K. Rowling",
      "average_rating": 4.73,
      "isbn": 439827604,
      "language_code": "eng",
      "ratings_count": 27410,
      "price": 237
    },
    {
      "bookID": 12,
      "title": "The Ultimate Hitchhiker's Guide: Five Complete Novels and One Story (Hitchhiker's Guide to the Galaxy  #1-5)",
      "authors": "Douglas Adams",
      "average_rating": 4.38,
      "isbn": 517226952,
      "language_code": "eng",
      "ratings_count": 3602,
      "price": 238
    },
    {
      "bookID": 13,
      "title": "The Ultimate Hitchhiker's Guide to the Galaxy",
      "authors": "Douglas Adams",
      "average_rating": 4.38,
      "isbn": 345453743,
      "language_code": "eng",
      "ratings_count": 240189,
      "price": 239
    },
    {
      "bookID": 14,
      "title": "The Hitchhiker's Guide to the Galaxy (Hitchhiker's Guide to the Galaxy  #1)",
      "authors": "Douglas Adams",
      "average_rating": 4.22,
      "isbn": 1400052920,
      "language_code": "eng",
      "ratings_count": 4416,
      "price": 240
    },
    {
      "bookID": 16,
      "title": "The Hitchhiker's Guide to the Galaxy (Hitchhiker's Guide to the Galaxy  #1)",
      "authors": "Douglas Adams-Stephen Fry",
      "average_rating": 4.22,
      "isbn": 739322206,
      "language_code": "eng",
      "ratings_count": 1222,
      "price": 241
    },
    {
      "bookID": 18,
      "title": "The Ultimate Hitchhiker's Guide (Hitchhiker's Guide to the Galaxy #1-5)",
      "authors": "Douglas Adams",
      "average_rating": 4.38,
      "isbn": 517149257,
      "language_code": "en-US",
      "ratings_count": 2801,
      "price": 242
    },
    {
      "bookID": 21,
      "title": "A Short History of Nearly Everything",
      "authors": "Bill Bryson-William Roberts",
      "average_rating": 4.2,
      "isbn": 234234,
      "language_code": "eng",
      "ratings_count": 228522,
      "price": 243
    },
    {
      "bookID": 22,
      "title": "Bill Bryson's African Diary",
      "authors": "Bill Bryson",
      "average_rating": 3.43,
      "isbn": 767915062,
      "language_code": "eng",
      "ratings_count": 6993,
      "price": 244
    },
    {
      "bookID": 23,
      "title": "Bryson's Dictionary of Troublesome Words: A Writer's Guide to Getting It Right",
      "authors": "Bill Bryson",
      "average_rating": 3.88,
      "isbn": 767910435,
      "language_code": "eng",
      "ratings_count": 2020,
      "price": 245
    },
    {
      "bookID": 24,
      "title": "In a Sunburned Country",
      "authors": "Bill Bryson",
      "average_rating": 4.07,
      "isbn": 767903862,
      "language_code": "eng",
      "ratings_count": 68213,
      "price": 246
    },
    {
      "bookID": 25,
      "title": "I'm a Stranger Here Myself: Notes on Returning to America After Twenty Years Away",
      "authors": "Bill Bryson",
      "average_rating": 3.9,
      "isbn": 243324,
      "language_code": "eng",
      "ratings_count": 47490,
      "price": 247
    },
    {
      "bookID": 26,
      "title": "The Lost Continent: Travels in Small Town America",
      "authors": "Bill Bryson",
      "average_rating": 3.83,
      "isbn": 60920084,
      "language_code": "en-US",
      "ratings_count": 43779,
      "price": 248
    },
    {
      "bookID": 27,
      "title": "Neither Here nor There: Travels in Europe",
      "authors": "Bill Bryson",
      "average_rating": 3.87,
      "isbn": 380713802,
      "language_code": "eng",
      "ratings_count": 46397,
      "price": 249
    },
    {
      "bookID": 28,
      "title": "Notes from a Small Island",
      "authors": "Bill Bryson",
      "average_rating": 3.92,
      "isbn": 380727501,
      "language_code": "eng",
      "ratings_count": 76476,
      "price": 250
    },
    {
      "bookID": 29,
      "title": "The Mother Tongue: English and How It Got That Way",
      "authors": "Bill Bryson",
      "average_rating": 3.94,
      "isbn": 380715430,
      "language_code": "eng",
      "ratings_count": 26672,
      "price": 251
    },
    {
      "bookID": 30,
      "title": "J.R.R. Tolkien 4-Book Boxed Set: The Hobbit and The Lord of the Rings",
      "authors": "J.R.R. Tolkien",
      "average_rating": 4.59,
      "isbn": 345538374,
      "language_code": "eng",
      "ratings_count": 97731,
      "price": 252
    }
  ]
  ;

  pageTitle = 'Book List';
  filteredBooks: IBook[] = [];
  books: IBook[] = [];
  cart = new Set<number>();
  starwidth: number;
  searchTerm: string = "Analysis";

  constructor(private bookService: BookService) { }

  sortedBooks(): IBook[] {
    return this.localBooks.sort((a: IBook, b: IBook) => b.average_rating - a.average_rating);
  }
  addToCart(bookID: number): boolean {
    for (const book of this.books) {
      if (book.bookID === bookID) {
        this.cart.add(book.bookID);
        break;
      }
    }
    return false;
  }
  searchBook(): boolean {
    console.log(this.localBooks.length);
    this.searchTerm = this.searchTerm.trim().toLocaleLowerCase();
    this.localBooks =  this.localBooks.filter((book: IBook) =>
      book.title.toLocaleLowerCase().indexOf(this.searchTerm) !== -1);
    console.log(this.localBooks.length);
    return false;
  }
  ngOnInit(): void {
    this.books = this.sortedBooks();
    // this.bookService.getBooks().subscribe(
    //   books => {
    //     this.books = books;
    //   }
    // );
  }

}
